  ...
  return (
    <div className="container-page py-10">
      ...
      <div className="grid md:grid-cols-2 gap-4 mb-4">
        <GenerationHeader g={a} accent="from-brand-100 to-brand-50" />
        <GenerationHeader g={b} accent="from-rose-100 to-rose-50" />
      </div>
      ...
      {(!pa || !pb) ? (
        ...
        <>
          <div className="card p-5 mb-6">
            <div className="text-xs uppercase tracking-wide text-ink-500 font-semibold mb-1">Écart générationnel</div>
...
function Picker({
  ...
  return (
    <label className="card p-4 block">
      <span className={`text-xs uppercase tracking-wide font-semibold ${accent === 'brand' ? 'text-brand-700' : 'text-rose-700'}`}>
        {label}
      </span>
      <select
        value={value}
        onChange={e => onChange(e.target.value)}
        className="mt-2 w-full rounded-lg border border-ink-200 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500"
      >
        {GENERATIONS.map(g => (
...
function GenerationHeader({ g, accent }: { g: Generation; accent: string }) {
  return (
    <div className={`rounded-2xl p-5 bg-gradient-to-br ${accent} border border-ink-100`}>
      <div className="flex items-center gap-3">
        <div className="text-4xl">{g.emoji}</div>
        <div>
          <div className="text-xs text-ink-600">{g.yearRange[0]}–{g.yearRange[1]}</div>
          <div className="font-display font-bold text-lg">{g.name}</div>
        </div>
      </div>
      <p className="mt-2 text-sm text-ink-700">{g.tagline}</p>
    </div>
  )
}
...
function ContextRow({
  ...
  return (
    <div className="card overflow-hidden">
      <div className="px-5 py-3 bg-ink-100 flex items-center gap-2">
    ...
    if (dimension.id === 'valeurs') {
      return (
        <div className="flex flex-wrap gap-1.5">
          {p.valeurs.map(v => (
            <span key={v} className="chip bg-ink-100 text-ink-800">{v}</span>
          ))}
        </div>
      )
    }
  ...
  return (
    <div className="card overflow-hidden">
      <div className="px-5 py-3 bg-ink-100 flex items-center gap-2">
        ...
        <span className="text-xs text-ink-500 ml-2 hidden sm:inline">{dimension.description}</span>
      </div>
      <div className="grid md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-ink-100">
...
function Cell({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="p-5">
      <div className="text-xs uppercase tracking-wide text-ink-500 font-semibold mb-1">{label}</div>
