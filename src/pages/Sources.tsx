      ...
      <div className="card divide-y divide-ink-100">
        {list.map(s => (
          <div key={s.id} className="p-4 flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-4">
            <div className="text-sm font-semibold text-ink-900 w-56 shrink-0">
              {s.org}
              {s.year && <span className="text-ink-500 font-normal"> · {s.year}</span>}
            </div>
            <div className="flex-1">
              <a href={s.url} target="_blank" rel="noreferrer" className="text-sm text-brand-700 hover:underline">
                {s.label}
              </a>
            </div>
          </div>
