    ...
    <div className="container-page py-10">
      <section className="grid lg:grid-cols-5 gap-8 items-center">
        <div className="lg:col-span-3">
          <div className="chip bg-brand-100 text-brand-800 mb-4">v0.1 — édition francophone</div>
          <h1 className="text-4xl sm:text-5xl font-display font-extrabold leading-tight text-ink-900">
          ...
          <div className="mt-6 text-sm text-ink-500">
            Région active : <span className="font-semibold text-ink-700">{r.flag} {r.label}</span>
          </div>
        </div>
        <div className="lg:col-span-2">
          <div className="card p-5 bg-gradient-to-br from-brand-50 to-white">
            <div className="font-display font-bold text-ink-900 mb-2">Pourquoi cet outil ?</div>
      ...
      <section className="mt-14">
        <div className="flex items-end justify-between mb-5">
          ...
          <Link to="/generations" className="text-sm text-brand-700 hover:underline">Tout voir →</Link>
        </div>
...
function FeatureCard({ to, icon, title, desc }: { to: string; icon: string; title: string; desc: string }) {
  return (
    <Link to={to} className="card p-5 hover:shadow-md transition">
      ...
      <div className="mt-2 font-display font-bold text-ink-900">{title}</div>
